...
class GAT(nn.Module):
    def __init__(self,
        ...
        self.norm_layers.append(nn.BatchNorm1d(args.GAT_args["num_hidden"]*heads[0]))
        ...
        for l in range(1, args.GAT_args['num_layers']):
            ...
            self.norm_layers.append(nn.BatchNorm1d(args.GAT_args["num_hidden"]*heads[l]))
            # self.norm_layers.append(PairNorm())
        # output projection
    ...
    def forward_batch(self, blocks, features):
        ...
        num_layers = len(self.gat_layers) - 1
        if len(blocks) != num_layers + 1:
            raise ValueError(f"GAT has {num_layers + 1} layers (including output), but got {len(blocks)} blocks. "
                           f"Please ensure eval_n_nbs_sample has {num_layers + 1} values.")
        

        for i, layer in enumerate(self.gat_layers[:-1]):

            num_src = blocks[i].num_src_nodes()
            if h.shape[0] != num_src:
                raise ValueError(f"Layer {i}: Expected {num_src} source nodes in block, but got {h.shape[0]} features. "
                               f"Block has {blocks[i].num_src_nodes()} src nodes and {blocks[i].num_dst_nodes()} dst nodes.")
            h, e = layer.forward_batch(blocks[i], h)

            h = h.flatten(1)  # [num_dst_nodes, num_hidden * num_heads]
            h = self.activation(h)
            e_list = e_list + e
        

        num_src_last = blocks[-1].num_src_nodes()
        num_dst_prev = blocks[-2].num_dst_nodes() if len(blocks) > 1 else blocks[0].num_dst_nodes()
        

        if h.shape[0] != num_src_last:

            if h.shape[0] == num_dst_prev and num_dst_prev == num_src_last:
                pass
            else:

                if 'feat' in blocks[-1].srcdata or 'feature' in blocks[-1].srcdata:
                    feat_key = 'feat' if 'feat' in blocks[-1].srcdata else 'feature'
                    h_from_block = blocks[-1].srcdata[feat_key]

                    if h_from_block.shape[0] == num_src_last and h_from_block.shape[1] == h.shape[1]:
                        h = h_from_block
                    else:
                        raise ValueError(f"Output layer: Feature dimension mismatch. "
                                       f"Expected {num_src_last} source nodes, got {h.shape[0]} features. "
                                       f"Previous layer output: {h.shape[0]}, block src nodes: {num_src_last}, "
                                       f"block dst nodes (prev): {num_dst_prev}")
                else:
                    raise ValueError(f"Output layer: Feature dimension mismatch. "
                                   f"Expected {num_src_last} source nodes, got {h.shape[0]} features. "
                                   f"Previous layer output: {h.shape[0]}, block src nodes: {num_src_last}, "
                                   f"block dst nodes (prev): {num_dst_prev}. "
                                   f"Block has no 'feat' or 'feature' in srcdata.")
        logits, e = self.gat_layers[-1].forward_batch(blocks[-1], h)
        self.second_last_h = logits if len(self.gat_layers) == 1 else h
        logits = logits.mean(1)
        e_list = e_list + e
