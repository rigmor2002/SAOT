...
class GINConv(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        graph.update_all(fn.copy_src('h', 'm'), self._reducer('m', 'neigh'))
    ...
    def forward_batch(self, block, feat):
        ...
        feat_dst = feat_dst.contiguous()
        block.dstdata['h'] = feat_dst

        block.update_all(fn.copy_src('h', 'm'), self._reducer('m', 'neigh'))
...
class GCNLayer(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        h = self.linear(feat)
        h = h * norm  # 应用输入归一化 D^-0.5 * X * W
        
        graph.ndata['h'] = h
        graph.update_all(gcn_msg, gcn_reduce)
        h = graph.ndata['h']
        h = h * norm  # 应用输出归一化 D^-0.5 * A * D^-0.5 * X * W
        # --- GCN归一化修改结束 ---
    ...
    def forward_batch(self, block, feat):
        ...
        # 修复：如果 feat 的维度不匹配 block 的源节点数，尝试从 block.srcdata 中获取
        # 在 DGL block 采样中，feat 应该对应所有源节点
        num_src_nodes_in_block = block.num_src_nodes()
        if feat.shape[0] != num_src_nodes_in_block:
            # 如果维度不匹配，尝试从 block.srcdata 中获取特征
            if 'feat' in block.srcdata:
                feat = block.srcdata['feat']
            elif 'feature' in block.srcdata:
                feat = block.srcdata['feature']
            else:
                raise ValueError(f"Feature dimension mismatch: feat has {feat.shape[0]} nodes, "
                               f"but block has {num_src_nodes_in_block} source nodes. "
                               f"And no 'feat' or 'feature' found in block.srcdata.")
        
        feat_src, feat_dst = expand_as_pair(feat, block)
        
        # 计算源节点和目标节点的度归一化因子
        num_src_nodes = feat_src.shape[0]
        num_dst_nodes = feat_dst.shape[0]
        
        # 获取所有源节点和目标节点的度
        all_degs_src = block.out_degrees(block.srcnodes()).float().clamp(min=1)
        all_degs_dst = block.in_degrees(block.dstnodes()).float().clamp(min=1)
        
        # 确保维度匹配
        if all_degs_src.shape[0] != num_src_nodes:
            all_degs_src = all_degs_src[:num_src_nodes]
        if all_degs_dst.shape[0] != num_dst_nodes:
            all_degs_dst = all_degs_dst[:num_dst_nodes]
        
        degs_src = all_degs_src
        degs_dst = all_degs_dst
        
        norm_src = th.pow(degs_src, -0.5).to(feat.device).unsqueeze(1)
        norm_dst = th.pow(degs_dst, -0.5).to(feat.device).unsqueeze(1)
        
        # 线性变换并归一化源特征
        h_src = self.linear(feat_src)
        h_src = h_src * norm_src
        ...
        # 获取聚合后的目标节点特征并归一化
        h = block.dstdata.pop('h') # 这里 pop 出来了
        h = h * norm_dst
        
        # --- 【修复】 将归一化后的 h 放回 dstdata，供 apply_edges 使用 ---
        block.dstdata['h'] = h 
        # -----------------------------------------------------------

        # 现在 apply_edges 可以同时找到 src['h'] 和 dst['h'] 了
        block.apply_edges(lambda edges: {'e': th.sum((th.mul(edges.src['h'], th.tanh(edges.dst['h']))), 1)})
    ...
    #     # 计算源节点和目标节点的度归一化因子
    #     degs_src = block.out_degrees(block.srcnodes()).float().clamp(min=1)
    #     norm_src = th.pow(degs_src, -0.5).to(feat.device).unsqueeze(1)
        
    #     degs_dst = block.in_degrees(block.dstnodes()).float().clamp(min=1)
    #     norm_dst = th.pow(degs_dst, -0.5).to(feat.device).unsqueeze(1)
    ...
    #     # 消息传递：只设置 srcdata，让 DGL 自动聚合到 dstdata
    #     block.srcdata['h'] = h_src
    #     block.update_all(gcn_msg, gcn_reduce)
