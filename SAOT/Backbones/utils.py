...
class incremental_graph_trans_(nn.Module):
    ...
    def get_graph(self, tasks_to_retain=[], node_ids = None, remove_edges=True):
        ...
        # Optimization: Use a lookup tensor for O(1) mapping instead of O(N) search
        # Products-CL has ~2.4M nodes, searching linearly for each train node is impossibly slow
        max_id = self.graph.num_nodes()
        mapping = torch.full((max_id + 1,), -1, dtype=torch.long)
        mapping[old_ids] = torch.arange(len(old_ids), dtype=torch.long)
        
        ids_train = mapping[ids_train_old].tolist()
        ids_val = mapping[ids_valid_old].tolist()
        ids_test = mapping[ids_test_old].tolist()
        
        # Verify no invalid mappings (optional, can be removed for speed)
        # if -1 in ids_train or -1 in ids_val or -1 in ids_test:
        #    raise ValueError("Some IDs were not found in the subgraph")
