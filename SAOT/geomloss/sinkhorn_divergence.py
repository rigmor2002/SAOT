...
def epsilon_schedule(p, diameter, blur, scaling):
    ...
    eps_list = (
        ...
        + [
            np.exp(e)
            for e in np.arange(
                p * np.log(max(diameter, 1e-6)), p * np.log(blur), p * np.log(scaling)
            )
        ]
        + [blur ** p]
    )
...
def scaling_parameters(x, y, p, blur, reach, diameter, scaling):
    ...
    if diameter <= 0 or not np.isfinite(diameter) or type(diameter) is torch.Tensor and diameter.item() <= 0:
        diameter = 1e-4
    if blur <= 0:
        blur = 1e-4
...
def sinkhorn_loop(
    ...
    for i, eps in enumerate(eps_list):  # See Fig. 3.25-26 in Jean Feydy's PhD thesis.
        ...
        if debias:
            f_aa, g_bb = 0.5 * (f_aa + ft_aa), 0.5 * (g_bb + gt_bb)  # OT
